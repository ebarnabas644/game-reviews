...
export class GameDataService {
  ...
  private gameUpdateSchedulePath = '/api/appList/updateStatus/';
  ...
  gameData$ = this.parameterChangeAction$.pipe(
    switchMap((params) =>
      this.http.get<AppDetail[]>(
        this.gameDataPath + `/getBatch/${this.currentIndex}`,
        {
          params: {
            l: this.languagePipe.transform(params.language),
            size: this.batchSize,
            name: `${params.name}`,
            ...
            min_metacritic_score:
              params.min_metacritic_score == -1
                ? ''
                : params.min_metacritic_score,
  ...
  addGameToUpdateSchedule(appid: number) {
    return this.http.get<any>(
      this.apiBaseUrl + this.gameUpdateSchedulePath + `${appid}&true`
    );
  }
}
